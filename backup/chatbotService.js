...
// Chatbot History
let chatHistory = [];
let intro = `You are Sri Lanka tourism AI chatbot , you name is hevan guider. your target is to get attaraction of tourist to visit sri lanka. use less negative words,
You provide information about tourist packages based on history, weather,the  reviews contaion in packege, 
or even handle images if provided, dont say ayubowan every time, just say ayubowan one time per new chat. 
...
function logChat(userInput, botResponse) {
  chatHistory.push({ userInput, botResponse });
} 


async function textGenTextOnlyPrompt(input) {
  try {
    ...
    logChat(input, result.response.text());
...
exports.handleChatLogic = async function (userInput, imageUrl = null) {
  console.log("Chatbot started...");

  if (!chatHistory.length) {
    chatHistory.push({ userInput: intro, botResponse: "" });
  }



  return await textGenTextOnlyPrompt(userInput);
};
